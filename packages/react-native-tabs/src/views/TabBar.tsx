...
import { Route, SceneRendererProps, NavigationState } from 'react-native-tab-view';

import TabBarItem, { Props as TabBarItemProps } from './TabBarItem';
import TabBarIndicator, { Props as IndicatorProps } from './TabBarIndicator';
import { Scene, Layout, Event } from '../types';
...
export default class TabBar<T extends Route> extends React.Component<Props<T>, State> {
  static defaultProps = {
    ...
    getAccessible: ({ route }: Scene<Route>) => (typeof route.accessible !== 'undefined' ? route.accessible : true),
    ...
    renderIndicator: (props: IndicatorProps<Route>) => <TabBarIndicator {...props} />,
  };
  ...
  componentDidUpdate(prevProps: Props<T>, prevState: State) {
    ...
    if (
      prevProps.navigationState.routes.length !== navigationState.routes.length ||
      ...
      if (
        ...
        !(layout.width && navigationState.routes.every(r => typeof tabWidths[r.key] === 'number'))
      ) {
  ...
  private getMaxScrollDistance = (tabBarWidth: number, layoutWidth: number) => tabBarWidth - layoutWidth;
  ...
  private getTabBarWidth = (props: Props<T>, state: State) => {
    ...
    return routes.reduce<number>(
      (acc, _, i) =>
        acc +
        this.getComputedTabWidth(i, layout, routes, scrollEnabled, tabWidths, this.getFlattenedTabWidth(tabStyle)),
      0
    );
  };

  private normalizeScrollValue = (props: Props<T>, state: State, value: number) => {
    const { layout } = state;
  ...
  private getScrollAmount = (props: Props<T>, state: State, index: number) => {
    ...
    const centerDistance = Array.from({ length: index + 1 }).reduce<number>((total, _, i) => {
  ...
  private getTranslateX = (scrollAmount: Animated.Value, maxScrollDistance: number) =>
    Animated.multiply(
      ...
      <Animated.View onLayout={this.handleLayout} style={[styles.tabBar, style]}>
        <Animated.View
          ...
          style={[
            ...
            tabBarWidth ? { width: tabBarWidth } : scrollEnabled ? { width: tabBarWidthPercent } : null,
            indicatorContainerStyle,
          ]}
        >
        ...
        <View style={styles.scroll}>
          <Animated.ScrollView
            ...
            contentContainerStyle={[
              styles.tabContent,
              scrollEnabled ? { width: tabBarWidth || tabBarWidthPercent } : styles.container,
              contentContainerStyle,
            ]}
            ...
            {routes.map((route: T) => {
              const props: TabBarItemProps<T> & { key: string } = {
                ...
                onLayout: isWidthDynamic
                  ? e => {
                      this.measuredTabWidths[route.key] = e.nativeEvent.layout.width;
