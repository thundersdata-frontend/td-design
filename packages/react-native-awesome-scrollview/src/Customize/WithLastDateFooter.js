...
import { NormalFooter } from '../NormalFooterer';
...
export class WithLastDateFooter extends NormalFooter {
  ...
  getTimeDescription() {
    ...
    const now = new Date();
    let timeInterval = Math.floor((now.getTime() - this.lastTime.getTime()) / 1000);
    if (timeInterval < 60) {
      ...
      return `${this.lastTime.getMonth() + 1}-${this.lastTime.getDate()}-${this.lastTime.getFullYear()}`;
    }
  }
}
