...
const getPointDistance = (a: number[], b: number[]) => Math.sqrt(Math.pow(a[0] - b[0], 2) + Math.pow(a[1] - b[1], 2));

export const getElementVisibleWidth = (elementWidth: number, xOffset: number, ScreenWidth: number) => {
...
const getTooltipCoordinate = (
  ...
  const qIndex = sortedArea[0].id; // 得到最小面积的id
  const getWithPointerOffsetX = () => center[0] - 18 * directionCorrection[qIndex][0]; // 判断X轴偏移方向得到偏移量
  const getWithPointerOffsetY = () => 10 * directionCorrection[qIndex][1]; // 判断Y轴偏移方向得到偏移量
...
const constraintX = (newX: number, qIndex: number, _x: number, ScreenWidth: number, tooltipWidth: number) => {
  switch (qIndex) {
